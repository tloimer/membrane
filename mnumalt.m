...
if isfinite(ps1) % above the critical point, these numbers do not make sense
  fl.calc = struct('psat1',ps1,'pK1',[],'n',[],'Ccc',[],'Ccap',[],...
    'kappac',[],'kapK',[],'kapl',[],'mlinp1sat',[]);
  [dpk1 fl.calc.pK1] = flsetup.dpkdT(T1);
  fl.calc.n = s.jt(T1,p1)*dpk1;
  fl.calc.kappac = s.nul(T1)*flsetup.kmliq(T1)/s.hvap(T1); % reuse kappac!
  fl.calc.kapK = fl.calc.kappac/dpk1;
  [dsig sigma] = s.dsig(T1);  dsig = dsig*sigma;
  fl.calc.kapl = fl.calc.kappac/(dpk1-flsetup.curv*dsig);
  fl.calc.kappac = fl.calc.kappac/dps1; % kappac now is OK.
  if theta ~= 90
    if theta < 90
      fl.calc.Ccc = p12*(1-fl.calc.n)/(ps1-fl.calc.pK1);
    else
      fl.calc.Ccap = fl.calc.n*p12/(flsetup.curv*sigma);
    end
  end
% Calculate the mass flux for p1 = psat(T1) according to linear theory.
  flin = mlinear(ps1,ps1-p12,T1,theta,s,mem,f);
  fl.calc.mlinp1sat = flin.lin.m;
end % isfinite(ps1)
% Calculated, fl.calc written.
...
if nargin < 9
  if p1 >=  flsetup.pkelv(T1) % fl.calc.pK1 is not set for gases
