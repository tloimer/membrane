function flsetup = curvsetup(T2,Tmax,curv,s,mem,f)
%CURVSETUP  Setup flow properties, given radius of curvature.
%  CURVSETUP(T2,TMAX,CURV,S,MEM,F) returns a struct FS that contains flow
%  properties for a combination of a radius of curvature CURV, a
...
%
%  FREEFLOW = CURVSETUP(S) sets the flowstruct FREEFLOW to properties
...
if curv == 0 || isfreespace || issupercritical
  flsetup.curv = 0;
  ...
  % do not return for theta == 90, curv == 0
else
  flsetup.curv = curv;
