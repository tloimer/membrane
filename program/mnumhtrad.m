function [m,ms] = mnumhtrad(alpha,T1,p1,p2,s,ms,accuracy)
%MNUMHTRAD   Mass flux for radiation flux and heat transfer coefficient.
%  MNUMHTRAD(ALPHA,T1,P1,P2,SUBSTANCE,MS) returns the mass flux [kg/m2s] for
%  adiabatic flow of SUBSTANCE through a stack of layered membranes MS. At the
%  downstream side of the struct, the sum of black body radiation and a heat
%  flux due to a heat transfer coefficent between downstream and upstream
%  temperature applies,
%
%    q2 = -ALPHA * (T1 - T2) - sigma * (T1^4 - T2^4),
%
...
%  Hence, the surroundings are all assumed to be at upstream temperature T1.
%  The membrane struct MS is constructed with MSTACKSTRUCT.
%
%  See MNUMRADIATION for radiation heat exchange.
%
%  MNUMHTRAD(ALPHA,T1,P1,P2,SUBSTANCE,MS,'crude') sets crude solver tolerances.
%
%  [M,MS] = MNUMHTRAD(ALPHA,T1,P1,P2,SUBSTANCE,MS) writes the solution to MS.
%
%  Calls ASYM.
%
...
function pres = presiduum(m) %---------------------------------------- presiduum
	...
	% q2 = m (h1 - h2) = alpha (T2 - T1) + sigma (T2^4 - T1^4)
	% I believe, numerically it is more accurate to write
	% T1^4 - T^4 = (T1 - T)(T1 + T)(T1^2 + T^2)
	T2 = fzero(@(T) alpha * (T1 - T) + ...
			5.67e-8 * (T1 - T) * (T1 + T) * (T1^2 + T^2) + ...
			m*h12(T),...
		   [Tmin T1]);
	state2.T = T2;
	state2.q = m * h12(T2);
...
end %%% END MNUMHTRAD %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% END MNUMHTRAD %%%
