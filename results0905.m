...
[kappa kappac kapK kapl psat1 pK1 T2calc n Ccc mlinp1sat ...
  mexp mgas miso m90 mcalc] = deal(kap_kc);
...
while first < lendata
  ...
  for i = first:last
    ...
    try
      ...
      fl.info = rmfield(fl.info,'substance');
      fl90{i} = fl;
    ...
    if ~strcmp(sname,'nitrogen') && size(fl0,2)==i
      ...
      kappa(i) = mem.kappa;
      kapK(i) = fl0{i}.calc.kapK;
      kapl(i) = fl0{i}.calc.kapl;
...
save(file,'kap_kc','kappac','psat1','pK1','T2calc','mexp','mgas','m90',...
  'miso','mcalc','fl0','fl90','kappa','kapK','kapl','n','Ccc','mlinp1sat');
