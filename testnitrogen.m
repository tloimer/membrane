...
for m = 1:size(memname,1)
  ...
  mem = membrane(poredia(m),eps(m),1.38,topology,tau,beta,L(m));
  ...
  k = mean(data./calc2);
  k = max(1,round(100*k))/100;
  plot(pred,data,'k*',pred,calc2*k,'k+-');
  title(sprintf('{\\bf%s}.  calc. \\times %.2f.', memname{m},k));
  xlabel(xlabelstr);
