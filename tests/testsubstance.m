...
first = find(allT>140,1);
nonarrayplot(s.ps,'Saturation pressure',...
   len+1-first,allT(first:len,1),'T [K]',satdata(first:len,2)*1e6);

% Liquid density
nonarrayplot(s.rho,'Density of the liquid',len,allT,'T [K]',satdata(:,3));

last = find(allT>330,1);
last = last - 1;
if VERBOSE
  datam = zeros(last,1);
  for i = 1:last
    datam(i,1) = s.hvap(allT(i,1));
  end
  fprintf(...
    'Enthalpy of vaporization:\n T [K] h_liq h_vap Dh_vap Dh_vap(matlab)\n');
  [satdata(1:last,[1,6,18]) satdata(1:last,18)-satdata(1:last,6) datam*1e-3]
end
% enthalpy of vaporization
nonarrayplot(s.hvap,'Enthalpy of vaporization',...
  last,allT(1:last,1),'T [K]',(satdata(1:last,18)-satdata(1:last,6))*1e3);

% Specific volume of the vapor
pplot(s.v,'Specific volume of the vapor',last,...
  allT(1:last,1),satdata(1:last,2)*1e6,'T [K]',satdata(1:last,16));

% Joule-Thomson coefficient
first = 1; %find(allT>150,1);
if VERBOSE
  datam = zeros(last+1-first,1);
  for i = 1:last+1-first
    datam(i,1) = s.jt(allT(first-1+i,1),satdata(first-1+i,2)*1e6);
  end
  fprintf(...
    'Joule-Thomson coefficient:\n T [K] JT [K/MPa] JT(matlab) [K/MPa]\n');
    datam2 = s.jt(allT(first:last,1),satdata(first:last,2)*1e6);
    [satdata(first:last,[1,23]) datam*1e6 datam2*1e6]
end
pplot(s.jt,'Joule-Thomson coefficient',last+1-first,allT(first:last,1),...
  satdata(first:last,2)*1e6,'T [K]',satdata(first:last,23)*1e-6);

% Specific isobaric heat capacity of the vapor
last = find(allT>310,1);
last = last - 1;
pplot(s.cpg,'Specific isobaric heat capacity of the vapor',last,...
  allT(1:last,1),satdata(1:last,2)*1e6,'T [K]',satdata(1:last,21)*1e3);

% Specific isobaric heat capacity in the ideal gas stat
pplot(s.cpg,'Isobaric heat capacity, ideal gas state',size(p1Pa,1),...
  p1Pa(:,1),p1Pa(:,2)*1e6,'T [K]',p1Pa(:,9)*1e3);
% Specific isobaric heat capacity of the liquid
arrayplot(s.cpl,'Specific isobaric heat capacity of the liquid',len,...
  allT,'T [K]',satdata(:,9)*1e3);

% Surface tension
arrayplot(s.sigma,'Surface tension',len,allT,'T [K]',satdata(:,14));

% Dynamic viscosity of the liquid
arrayplot(s.mul,'Dynamic viscosity of the liquid',len,...
  allT,'T [K]',satdata(:,12));

% Dynamic viscosity of the vapor
arrayplot(s.mug,'Dynamic viscosity of the vapor',len,...
  allT,'T [K]',satdata(:,24));

% Thermal conductivity of the vapor
arrayplot(s.kg,'Thermal conductivity of the vapor',len,...
  allT,'T [K]',satdata(:,25));

% Thermal conductivity of the liquid
arrayplot(s.kl,'Thermal conductivity of the liquid',len,...
  allT,'T [K]',satdata(:,13));

last = find(allT>330,1);
last = last - 1;
% Kinematic viscosity of the vapor
pplot(s.nug,'Kinematic viscosity of the vapor',last,allT(1:last),...
  satdata(1:last,2)*1e6,'T [K]',satdata(1:last,24).*satdata(1:last,16));

% Kinematic viscosity of the liquid
nonarrayplot(s.nul,'Kinematic viscosity of the liquid',len,...
  allT,'T [K]',satdata(:,12).*satdata(:,4));


function nonarrayplot(prop,description,len,X,xname,datanist) %------------------

datam = zeros(len,1);
for i = 1:len
  datam(i,1) = prop(X(i,1));
end

figure('Name',description,'NumberTitle','Off');
plot(X,100*(datam./datanist-1),'k*');
title(description);
ylabel('100*(matlab - nist)/nist');
xlabel(xname);


function arrayplot(prop,description,len,X,xname,datanist) %---------------------

figure('Name',description,'NumberTitle','Off');
plot(X,100*(prop(X)./datanist-1),'k*');
title(description);
ylabel('100*(matlab - nist)/nist');
xlabel(xname);


function pplot(prop,description,len,X,P,xname,datanist) %-----------------------

figure('Name',description,'NumberTitle','Off');
plot(X,100*(prop(X,P)./datanist-1),'k*');
title(description);
ylabel('100*(matlab - nist)/nist');
xlabel(xname);

function pscalarplot(prop,description,len,X,P,xname,datanist) %-----------------

datam = zeros(len,1);
for i = 1:len
  datam(i,1) = prop(X(i,1),P(i,1));
end
